...
// Detect columns once the prerun has imported the file and bytes arrive (works for local + remote).
// immediate: true handles the case where bytes are already cached (e.g. reopening an existing block).
...
const setFile = (file: ImportFileHandle | undefined) => {
  ...
  // Setting fileHandle triggers the prerun, which imports the file and exposes it as a blob.
  // Once the bytes arrive via assayFileBytes, the watch above runs column detection.
